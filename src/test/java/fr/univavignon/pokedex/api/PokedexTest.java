...
public class PokedexTest {
    ...
    public void testGetPokemonMetadata() throws PokedexException {
        ...
        // Verify that the metadata retrieved matches the simulated metadata
        assertEquals(metadata, retrievedMetadata, "The metadata retrieved must match the simulated metadata.");
    }    
    ...
    // Test for covering the case where id >= 0 && id < pokemons.size() in getPokemon()
    @Test
    public void testGetPokemonValidIdBoundary() throws PokedexException {
        pokedex.addPokemon(pokemon1);
        Pokemon retrievedPokemon = pokedex.getPokemon(0); // Valid ID (boundary case)
        assertEquals(pokemon1, retrievedPokemon, "The Pokemon should be retrieved correctly for a valid ID.");
    }

    // Test for covering the case where id < 0 || id >= pokemons.size() in getPokemonMetadata()
    @Test
    public void testGetPokemonMetadataInvalidIdBoundary() {
        int invalidId = 999; // ID outside the valid range
